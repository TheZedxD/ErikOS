 ...
 */
// Authentication functions disabled - no login required for ErikOS
function showLoginScreen() {
  // Login screen disabled - ErikOS runs without authentication
  console.log("Login screen disabled");
}

function showPasswordPrompt(index) {
  // Password prompts disabled
  console.log("Password prompts disabled");
}

function showCreateProfileForm() {
  // Profile creation disabled
  console.log("Profile creation disabled");
}

function loginUser(profile) {
  // Login function disabled - user is always logged in
  console.log("Login function disabled");
}

function logoutUser() {
  // Logout disabled - cannot log out of ErikOS
  alert("Logout is disabled. ErikOS runs as a single-user system.");
}
...
async function openSettings() {
  ...
  // Security panel - disabled in single-user mode
  const securityPanel = makePanel("Security");
  const infoDiv = document.createElement("div");
  infoDiv.style.padding = "10px";
  infoDiv.innerHTML = "ErikOS runs as a single-user system. Authentication features are disabled.";
  securityPanel.append(infoDiv);
  // Audio panel
...
document.addEventListener("DOMContentLoaded", () => {
  // Load or create a default user (no authentication required)
  ...
  if (!profiles || profiles.length === 0) {
    ...
    currentUser = {
      ...
      showVolume: true,
      showLinks: true,
      visibleApps: applications.map((a) => a.id),
      conversations: [],
      portfolio: [],
      sheets: []
    };
    ...
    // Use the first profile as default (or find default)
    currentUser = profiles.find((p) => p.id === "default") || profiles[0];

    // Ensure all required properties exist
    if (!currentUser.iconLayout) currentUser.iconLayout = "grid";
    if (!currentUser.iconPositions) currentUser.iconPositions = {};
    if (currentUser.showClock === undefined) currentUser.showClock = true;
    if (currentUser.showVolume === undefined) currentUser.showVolume = true;
    if (currentUser.showLinks === undefined) currentUser.showLinks = true;
    if (!Array.isArray(currentUser.visibleApps))
      currentUser.visibleApps = applications.map((a) => a.id);
    if (!Array.isArray(currentUser.conversations)) currentUser.conversations = [];
    if (!Array.isArray(currentUser.portfolio)) currentUser.portfolio = [];
    if (!Array.isArray(currentUser.sheets)) currentUser.sheets = [];
  }

  // Record startup in logs
  addLog("ErikOS started");

  // Apply user settings (theme and wallpaper)
  applyUserSettings(currentUser);

  // Make sure desktop and taskbar are visible
  ...
  try {
    ...
    if (clockEl)
      clockEl.style.display = currentUser.showClock === false ? "none" : "flex";
    const volEl = document.getElementById("tray-volume-icon");
    if (volEl)
      volEl.style.display = currentUser.showVolume === false ? "none" : "inline";
    const linkEl = document.getElementById("tray-links-icon");
    if (linkEl)
      linkEl.style.display = currentUser.showLinks === false ? "none" : "inline";
  } catch (err) {
...
function openProfileManager() {
  ...
  nameLabel.innerHTML = "<strong>Name:</strong> " + currentUser.name;

  const themeLabel = document.createElement("div");
  themeLabel.innerHTML = "<strong>Theme:</strong> " + (currentUser.theme || "default");

  const infoText = document.createElement("div");
  infoText.style.marginTop = "20px";
  infoText.style.padding = "10px";
  infoText.style.border = "1px solid var(--window-border-dark)";
  infoText.innerHTML = "ErikOS runs as a single-user system. To change your name or theme, use the Settings app.";

  container.append(heading, nameLabel, themeLabel, infoText);
  windowManager.createWindow("profiles", "Profile", container);
}
